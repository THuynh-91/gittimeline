...
**How long a show runs** is derived, not fixed. Each visible commit is given 0.13 seconds of stage time — enough for its arrival to read as its own beat — and the history is collapsed into counted ribbons until what remains fits. The automatic length is therefore the size of what survives aggregation. Past **three minutes** the app asks before fetching, because a long show should be a choice; past **thirty-five** it stops stretching, because the alternative was measured and it is not a viewing option — Rust's 248,298 surviving nodes at a readable moment each came to 32,278 seconds, a nine-hour performance, and 645,561 camera keyframes to plan and carry. Below the ceiling nothing is compressed; above it, arrivals do get closer together, and that is the honest cost of the ceiling existing. Choosing an explicit length in Settings overrides the pace exactly.
