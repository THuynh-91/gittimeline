...
**The live path is the REST API, and it is what the product is.** Someone pastes a URL for a repository nobody has pre-fetched, and the browser reads its history directly from `api.github.com`: a hundred commits per request, ETag revalidation so an unchanged page costs nothing against the limit, an IndexedDB cache across visits, and a budget that stops before exhaustion and plays a truthful partial performance rather than a fabricated whole one. The ceiling is GitHub's — about sixty anonymous requests an hour, or five thousand with a token — and no browser application can move it. `README.md` covers what that means for a visitor; `docs/data-truth.md` covers what a partial fetch is allowed to claim.
