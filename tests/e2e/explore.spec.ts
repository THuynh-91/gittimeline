...
test.describe('travelling the finished picture', () => {
  test('a slider appears when the performance ends and pans without changing zoom', async ({ page }) => {
    ...
    await page.evaluate((t) => window.__gitdance.seek(t), dur);
    await expect(page.getByTestId('explore-bar')).toBeVisible();

    // Zoom in so there is something left to travel through.
    await page.evaluate(() => window.__gitdance.zoom(2.6));
    await page.waitForTimeout(120);
    const zoomed = await page.evaluate(() => window.__gitdance.viewport!.scale);

    const range = page.getByTestId('explore-range');
    await expect(range).toBeEnabled();
    await range.fill('40');
    await page.waitForTimeout(120);
    const left = await page.evaluate(() => window.__gitdance.viewport!);
    await range.fill('930');
    await page.waitForTimeout(120);
    const right = await page.evaluate(() => window.__gitdance.viewport!);
  ...
  test('the slider is keyboard operable and reports where it is', async ({ page }) => {
    ...
    const dur = await page.evaluate(() => window.__gitdance.duration);
    await page.evaluate((t) => window.__gitdance.seek(t), dur);
    await page.evaluate(() => window.__gitdance.zoom(3));
    await page.waitForTimeout(120);

    const range = page.getByTestId('explore-range');
    await expect(range).toHaveAttribute('aria-label', /zoom/i);
    await range.focus();
    const before = await page.evaluate(() => window.__gitdance.viewport!.cx);
    for (let i = 0; i < 12; i++) await page.keyboard.press('ArrowRight');
    await page.waitForTimeout(120);
    expect(await page.evaluate(() => window.__gitdance.viewport!.cx)).toBeGreaterThan(before);
  ...
  test('it disappears again once the performance is playing', async ({ page }) => {
    ...
    await page.evaluate((t) => window.__gitdance.seek(t), dur);
    await expect(page.getByTestId('explore-bar')).toBeVisible();
    await page.evaluate(() => window.__gitdance.seek(1));
    await expect(page.getByTestId('explore-bar')).toHaveCount(0);
  });
});
